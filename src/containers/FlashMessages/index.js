...
class FlashMessages extends React.Component {
  ...
  onAnimationEnd = (animName, item) => {
    if (animName === 'close') this.removeItem(item);
  ...
  setAllItemStatuses(status) {
    if (status === 'hovering') this.hoverActive = true;
    else this.hoverActive = false;
    const { statuses } = this.state;
    const newStatuses = {};
    ...
    for (let i = 0; i < keys.length; i += 1) {
      ...
      if (oldStatus !== 'clicked' && oldStatus !== 'opening') newStatuses[keys[i]] = status;
